...
<link rel="stylesheet" href="<%=cp%>/res/calendar/fullcalendar.min.css" type="text/css">
<link rel="stylesheet" href="<%=cp%>/res/calendar/fullcalendar.print.css" media='print' type="text/css">
...
<script type="text/javascript" src="<%=cp%>/res/calendar/lib/moment.min.js"></script>
...
function articleForm(calEvent) {
	...
	$('#scheduleModal .modal-body').load("<%=cp%>/club/${clubSeq}/calendar/articleForm", function() {
		$("#schTitle").html(title);
...
function insertForm(start, end) {
	$('#scheduleModal .modal-body').load("<%=cp%>/club/${clubSeq}/calendar/inputForm", function() {
...
function insertOk() {
	...
	var url="<%=cp%>/club/${clubSeq}/calendar/created";
...
function updateOk(num) {
	...
	var url="<%=cp%>/club/${clubSeq}/calendar/update";
...
function deleteOk(num) {
	if(confirm("삭제 하시겠습니까 ?")) {
		$.post("<%=cp%>/club/${clubSeq}/calendar/delete", {num:num}, function(data){
     	   var isLogin=data.isLogin;
    	   if(isLogin=="false") {
    		   location.href="<%=cp%>/member/login";
    		   return;
